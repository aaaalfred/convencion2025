...
/**
 * Generar URL firmada para acceder a imagen en S3
 * @param {string} s3Url - URL de S3 de la imagen
 * @param {number} expiresIn - Tiempo de expiración en segundos (default 1 hora)
 * @returns {string} - URL firmada
 */
function getPresignedUrl(s3Url, expiresIn = 3600) {
  if (!s3Url) return null;

  try {
    // Extraer el key de la URL de S3
    // URL típica: https://bucket-name.s3.region.amazonaws.com/faces/filename.jpg
    const urlObj = new URL(s3Url);
    const key = urlObj.pathname.substring(1); // Quitar el / inicial
    ...
    return s3Url; // Retornar URL original si falla
  }
}
